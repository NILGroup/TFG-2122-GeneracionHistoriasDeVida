...
def printAllText(triplesClusters, do_sample = False, num_beams = 1, no_repeat_ngram_size = 1, min_length = 0,
    ...
    The function returns a string of text
    
    :param triplesClusters: the triplesClusters object that we created earlier
    :param do_sample: If False, greedy decoding, if True, sampling, defaults to False (optional)
    :param num_beams: Number of beams for beam search. 1 means no beam search, defaults to 1 (optional)
    :param no_repeat_ngram_size: The size of ngrams that should not be repeated, defaults to 1
    (optional)
    :param min_length: The minimum length of the sequence to be generated, defaults to 0 (optional)
    :param max_length: The maximum length of the generated text, defaults to 500 (optional)
    :param top_k: The number of highest probability vocabulary tokens to keep for top-k-filtering.
    Between 1 and infinity. Default to 50, defaults to 50 (optional)
    :param top_p: The cumulative probability of parameter highest probability vocabulary tokens to keep
    when filtering the next token probabilities
    :param temperature: Controls the randomness of the text generation. Higher temperature means more
    randomness
    :param penalty: The penalty for the length of the text. The higher the penalty, the shorter the text
    :param num_beam_groups: Number of beam groups to use. If 1, then the beam search will be serialized.
    If > 1, then the beam search will be parallelized, defaults to 1 (optional)
    :param num_return_sequences: The number of sequences to return. If 1, then a single sequence is
    returned. If >1, then a batch of sequences is returned, defaults to 1 (optional)
...
def filteredTriplesByTags(triplesTree, themesSelection):
    """
    It takes a dictionary of dictionaries and a dictionary of lists, and returns a dictionary of
    dictionaries.
    
    The input dictionary of dictionaries is a tree of triples, where each triple is a dictionary with
    three keys: 'subject', 'predicate', and 'object'. The input dictionary of lists is a dictionary of
    lists of strings, where each string is a tag.
    
    The output dictionary of dictionaries is a tree of triples, where each triple is a dictionary with
    three keys: 'subject', 'predicate', and 'object'. The output dictionary of dictionaries is a subset
    of the input dictionary of dictionaries, where each triple has at least one tag in the input
    dictionary of lists
    
    :param triplesTree: a dictionary with the following structure:
    :param themesSelection: {'theme1': {'subtheme1': {'subsubtheme1': {}, 'subsubtheme2': {}},
    'subtheme2': {'subsubtheme1': {}, 'subsubtheme2': {}}}, 'theme2': {'subtheme1
    :return: A dictionary with the selected tags.
    """
...
def clusteringByTags(themesSelection):
    ...
    :param themesSelection: A dictionary
    """
...
def listTags2Tree(list_tags):
    """
    It takes a list of strings and returns a dictionary of dictionaries
    
    :param list_tags: a list of tags, each tag is a string separated by underscores
    :return: A dictionary with the tags as keys and the values are the tags that are children of the
    key.
    """
    list_dict_tags = []
    for lt in list_tags:
...
def network_graph(fileurl, tags):
    """
    It takes a list of tags and returns a plotly figure
    
    :param fileurl: the url of the file to be uploaded
    :param tags: list of strings, each string is a tag
    :return: A dictionary with the data and layout of the graph.
    """
    if(len(tags)==0):
        figure = {
...
def recursivetags(name, tags, pad, before_id, class_name):
    """
    It takes a dictionary of tags, and recursively creates a tree of buttons, with the id of each button
    being the path to that button
    
    :param name: the name of the tag
    :param tags: a dictionary of dictionaries, where the keys are the names of the tags, and the values
    are the children of that tag
    :param pad: the indentation of the tag
    :param before_id: the id of the parent tag
    :param class_name: the class of the button
    :return: A list
    """
...
def tags_network(fileUrl):
    """
    It takes a fileUrl as input, and returns a list of dictionaries, each of which represents a node in
    the network, and a list of unique ids for each node
    
    :param fileUrl: the url of the file you want to analyze
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    ...
                    dcc.Graph(id="grafo-conocimiento",figure=network_graph(FILEURL, [])),
            ...
            html.Div("hola3",id='hidden-div-selected', style={'display':'none'}),
...
@app.callback(
    ...
    Output('hidden-div-selected', 'className'),
...
@app.callback(
    ...
    State('entradaPersonalizada','value'),
    Input('generator','n_clicks'),
    State("hidden-div", "className"),
    State("hidden-div-selected", "className"),
    State('outputGenerator','value'),
    State('hidden-div_node', 'className'),
    State('do_sample','value'),      
    State('max_len', 'value'),
    State('top_p', 'value'),
    State('top_k', 'value'),
    State('temperature', 'value'),
    State('penalty', 'value'),
    State('num_beams', 'value'),
    State('no_repeat_ngram_size', 'value'),
    State('num_beam_groups','value'),
    State('experto','value'),
)
def display_output_text(contenido,n_clicks,tags,selected_tags,text,node, 
